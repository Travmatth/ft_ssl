...
void	salsa_round_1st(uint32_t state[16])
{
...
void	salsa_round_2nd(uint32_t state[16])
{
...
void	salsa_rounds(uint32_t state[16], int rounds)
{
	while (rounds)
	{
		salsa_round_1st(state);
		salsa_round_2nd(state);
		rounds -= 2;
	}
}

void	salsa_pre(uint32_t in[16], uint32_t tmp[16])
{
	tmp[0] = in[0];
	tmp[1] = in[1];
	tmp[2] = in[2];
